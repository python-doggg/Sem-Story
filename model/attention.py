...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            #print("hidden_states.shape", hidden_states.shape) # add hidden_states.shape torch.Size([6, 4096, 320])
            #print("image_hidden_states.shape", image_hidden_states.shape) # add 'NoneType'!
            #print("encoder_hidden_states.shape", encoder_hidden_states.shape) # add torch.Size([3, 77, 768])
            hidden_states, img_dif_condition = block( ######################## 3
                hidden_states,
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None: #if self.attn2 is not None: # 修改了
            norm_hidden_states_t = (
                self.norm2(hidden_states, timestep) if self.use_ada_layer_norm else self.norm2(hidden_states)
            )
            #print("norm_hidden_states_t.shape", norm_hidden_states_t.shape) # add torch.Size([6, 4096, 320])
            #print("encoder_hidden_states.shape", encoder_hidden_states.shape) # add torch.Size([3, 77, 768])
            #print("attention_mask.shape", attention_mask.shape) # add NoneType
        ...
        #else: # 20240722 add
            #hidden_states_t = hidden_states # 20240722 add
        ...
        if image_hidden_states is not None:
            ...
            #hidden_states = hidden_states_t + hidden_states_i # yuan you
        #else: # yuan you
            #hidden_states = hidden_states_t # yuan you
        if encoder_hidden_states is not None and image_hidden_states is not None:
