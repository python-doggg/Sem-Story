...
class UNet2DConditionOutput(BaseOutput):
    ...
    """
    sample: torch.FloatTensor # 4o 表示模型最后一层的输出隐藏状态
    img_dif_conditions: dict # 4o 包含与图像相关的条件信息
...
class UNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        #print("timesteps.shape",timesteps.shape) # add timesteps.shape torch.Size([1, 1])
        #print("sample.shape", sample.shape) # add sample.shape torch.Size([9, 4, 64, 64])
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                #print("sample.shape", sample.shape) # add torch.Size([6, 320, 64, 64])
                #print("emb.shape", emb.shape) # add torch.Size([6, 1280])
                ...
                sample, res_samples, down_img_dif_conditions = downsample_block( # 2
