...
class UNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.conv_in = nn.Conv2d(in_channels, block_out_channels[0], kernel_size=conv_in_kernel,
    ...
    def forward(
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            #print("laile", i, downsample_block)
            if i == 0 or i == 1:
        ...
        if self.mid_block is not None:
            sample, mid_img_dif_conditions = self.mid_block(
                sample,
                emb,
                image_hidden_states=None, # image_hidden_states=image_hidden_states
                encoder_hidden_states=encoder_hidden_states,
                cross_attention_kwargs=cross_attention_kwargs,
            )
            ...
            if i == 0 or i == 1:
                ...
                if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                    sample, up_img_dif_conditions = upsample_block(
                        ...
                        res_hidden_states_tuple=res_samples,
                        image_hidden_states=image_hidden_states,
                        encoder_hidden_states=None, # encoder_hidden_states=encoder_hidden_states
                        cross_attention_kwargs=cross_attention_kwargs,
                        upsample_size=upsample_size,
                    )
                    if len(up_img_dif_conditions) > 0:
