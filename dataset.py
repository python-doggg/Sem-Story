...
class StorySalonDataset(Dataset):
    def __init__(self, root, dataset_name):
        ...
        for key in keys:
            ...
            for video in self.train_image_folders: # video: image_folder, /dataset/image/00001
                images = sorted(os.listdir(video)) # images是列表
                if len(images) <= 3:
                    print(video)
                    continue
                else:
        ...
        '''
        self.video_image_dir = os.path.join("/mnt/lustre/pengjie/data/StorySalon/", 'Image_inpainted', 'Video') # yuan "./StorySalon/", 'image_inpainted_finally_checked'
        self.video_mask_dir = os.path.join("/mnt/lustre/pengjie/data/StorySalon/", 'Mask', 'Video') # yuan "./StorySalon/", 'mask'
        ...
        #self.train_image_folders.remove('/mnt/lustre/pengjie/data/StorySalon/Image_inpainted/.DS_Store') # add
        for video in self.train_image_folders: # video: image_folder, /dataset/image/00001
            ...
            if len(images) <= 3:
                ...
                for i in range(len(images) - 3):
                    self.train_image_list.append([os.path.join(video, images[i]), os.path.join(video, images[i+1]), os.path.join(video, images[i+2]), os.path.join(video, images[i+3])])
        #self.train_mask_folders.remove('/mnt/lustre/pengjie/data/StorySalon/Mask/.DS_Store')  # add
        for video in self.train_mask_folders: # video: mask_folder, /dataset/mask/00001
            ...
            if len(masks) <= 3:
                ...
                for i in range(len(masks) - 3):
                    self.train_mask_list.append([os.path.join(video, masks[i]), os.path.join(video, masks[i+1]), os.path.join(video, masks[i+2]), os.path.join(video, masks[i+3])])
        #self.train_text_folders = '/mnt/lustre/pengjie/data/StorySalon/Text/Caption/Video/' # add
        for video in self.train_text_folders: # video: image_folder, /dataset/image/00001
    ...
    def __getitem__(self, index):
        ...
        ref_image_ids = self.image_list[index][0:3] # 3 image_ids
        ...
        #return {"image": image, "text_input": "style"}
...
if __name__ == '__main__':
    train_dataset = StorySalonDataset(root="/mnt/lustre/pengjie/data/StorySalon/", dataset_name="test") # yuan train_dataset = COCOMultiSegDataset(root="./COCO2017/")
