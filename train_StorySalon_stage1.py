...
def train(
    ...
    #logdir += f"_{time_string}" # yuan wu v
    logdir += "global_bs2560"
    print('logdir', logdir)
    ...
    unet = UNet2DConditionModel.from_config(pretrained_model_path, subfolder="unet") # from_config yuan you
    pretrained_sdm = torch.load('/home/pengjie/StoryGen_c/ckpt/stable-diffusion-v1-5/unet/diffusion_pytorch_model.bin', map_location='cpu') # 'cpu'
    #print(f"Loaded pretrained SDM state dict: {type(pretrained_sdm)}, {len(pretrained_sdm)} keys")
    #unet = unet.load_SDM_state_dict(pretrained_sdm) yuan
    unet.load_SDM_state_dict(pretrained_sdm) # change
    ...
    train_dataset = StorySalonDataset(root="/mnt/lustre/pengjie/data/StorySalon/", dataset_name='train')
