...
class DualBlipDiffusion(LatentDiffusion):
        ...
        if 'inp_image' in c:
            ...
            if isinstance(c["subject_text"][0], list):
                ...
                style_ctx_embeddings = self.forward_ctx_embeddings(
                    input_image=inp_image, text_input=style_subject_text, blip=self.style_blip,
                    proj_layer=self.style_proj_layer
                )
                if self.training:
                ...
                content_ctx_embeddings = self.forward_ctx_embeddings(
                    input_image=inp_image, text_input=content_subject_text, blip=self.content_blip,
                    proj_layer=self.content_proj_layer
                )
                if self.training:
                ...
                ctx_embeddings = self.forward_ctx_embeddings(
                    input_image=inp_image, text_input=c["subject_text"], blip=self.style_blip,
                    proj_layer=self.style_proj_layer
                )
                ...
                text_encoder_hidden_states_style = text_encoder_hidden_states_content = self.cond_stage_model.encode(
                    c['target_text'])
            else:
                ctx_embeddings = self.forward_ctx_embeddings(
                    input_image=inp_image, text_input=c["subject_text"], blip=self.content_blip,
                    proj_layer=self.content_proj_layer
                )
                ...
                text_encoder_hidden_states_content = text_encoder_hidden_states_style = self.cond_stage_model.encode(
                    c['target_text'])
            ...
            encoder_hidden_states = [torch.zeros_like(text_encoder_hidden_states)[:, :self.num_query_token, :],
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, **kwargs):
        ...
        if self.training:
            c['drop_mask'] = (torch.rand(len(inp_image)) >= drop_subject_prob).reshape(len(inp_image), 1, 1).to(
                inp_image.device)[:bs]
        else:
        ...
        with torch.cuda.amp.autocast(dtype=amp_dtype, enabled=self.use_bf16):
            ...
            if self.model.conditioning_key is not None:
                if hasattr(self.cond_stage_model, "decode"):
                    ...
                    xc = log_txt_as_img((x.shape[3], x.shape[2]), batch[self.cond_stage_key],
                                        size=(x.shape[2] + x.shape[3]) // 64)
                    log["conditioning"] = xc
                elif self.cond_stage_key in ['class_label', "cls"]:
                    try:
                        xc = log_txt_as_img((x.shape[3], x.shape[2]), batch["human_label"],
                                            size=(x.shape[2] + x.shape[3]) // 64)
                        log['conditioning'] = xc
                    except KeyError:
